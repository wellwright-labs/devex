 ...
 * Focus: path construction with PULSE_DATA_DIR override
 */
...
Deno.test("getDataDir - uses PULSE_DATA_DIR when set", () => {
  const original = Deno.env.get("PULSE_DATA_DIR");
  try {
    Deno.env.set("PULSE_DATA_DIR", "/custom/path");
...
Deno.test("getDataDir - defaults to ~/.config/pulse", () => {
  const original = Deno.env.get("PULSE_DATA_DIR");
  try {
    Deno.env.delete("PULSE_DATA_DIR");
    const home = Deno.env.get("HOME");
    assertEquals(getDataDir(), `${home}/.config/pulse`);
  } finally {
    if (original) {
      Deno.env.set("PULSE_DATA_DIR", original);
    }
